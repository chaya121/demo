...
## 👤 วิธีเข้าใช้งานสำหรับผู้ใช้ทั่วไป

ส่วนนี้สำหรับพนักงาน (Mer / ฝ่ายผลิต / ฝ่ายขาย) ที่จะใช้งานระบบจริง **ไม่จำเป็นต้องรู้เรื่องโค้ดหรือติดตั้งอะไรเลย**

### เข้าใช้งาน
เปิดเบราว์เซอร์ (มือถือหรือคอมพิวเตอร์ก็ได้) แล้วเข้าลิงก์:

```
[ใส่ URL ของเว็บแอปที่ deploy บน Vercel แล้วตรงนี้]
```

> ระบบยังไม่มีหน้า login — ใครก็ตามที่มีลิงก์นี้เข้าใช้งานและแก้ไข/ลบข้อมูลได้ทันที ควรระวังเรื่องการแชร์ลิงก์ให้คนนอกทีม
>
> เว็บนี้เป็น PWA กดปุ่ม "เพิ่มลงหน้าจอโฮม" ในเบราว์เซอร์มือถือได้ เพื่อใช้งานเหมือนแอปทั่วไป

### เมนูหลัก 4 แท็บ

| แท็บ | ใช้ทำอะไร |
|---|---|
| 📋 กรอกใบขั้นตอนการผลิต | สร้าง/กรอกใบข้อมูลการผลิตใหม่ |
| 📥 ประวัติ/PDF | ดูใบที่บันทึกไว้ทั้งหมด, ดาวน์โหลด PDF, แก้ไข/ลบ, นำเข้า-ส่งออก Excel |
| 📊 สถิติ | สรุปภาพรวมจำนวนใบ/จำนวนที่ผลิต แยกตามแบรนด์และประเภทเสื้อผ้า |
| ⚙️ ข้อมูลหลัก | จัดการรายชื่อ Mer, แบรนด์, ประเภทเสื้อผ้า, ชิ้นส่วน, ขั้นตอนการเย็บ, เครื่องจักร |

### สร้างใบขั้นตอนการผลิตใหม่

1. เข้าแท็บ **📋 กรอกใบขั้นตอนการผลิต**
2. กรอกข้อมูลพื้นฐาน (วันที่, Mer, แบรนด์, ลูกค้า, ชื่อรุ่น, ประเภทเสื้อผ้า ฯลฯ) — ช่องแบบ dropdown พิมพ์ค้นหาได้เลย ถ้าพิมพ์ชื่อที่ยังไม่มีในลิสต์ จะมีตัวเลือก "➕ เพิ่ม" ให้กด เพื่อบันทึกชื่อนั้นไว้ใช้เลือกได้ในครั้งต่อไปทันที (ไม่ต้องไปเพิ่มที่หน้า "ข้อมูลหลัก" ก่อน)
3. แนบรูปสินค้าได้ที่ช่อง "📷 รูป และ รายละเอียดสินค้า" (แนบได้หลายรูป)
4. กรอกตารางขั้นตอนการผลิต — กด "➕ เพิ่มแถวขั้นตอน" แล้วเลือกชิ้นส่วน/ขั้นตอนการเย็บ/เครื่องจักร/เวลา/จำนวนคนงานของแต่ละขั้นตอน
5. กด **👁 ดูตัวอย่าง / บันทึก** เพื่อตรวจสอบข้อมูลทั้งหมดก่อนบันทึกจริง
6. ตรวจดูในหน้าตัวอย่างแล้วกด **💾 ยืนยันบันทึก** — ระบบจะสร้าง "เลขที่ใบงาน" (job_no) ให้อัตโนมัติ ไม่ต้องคิดเลขเอง

> ⚠️ ข้อมูลจะถูกบันทึกก็ต่อเมื่อกดยืนยันบันทึกแล้วเท่านั้น ถ้าปิดหน้าเว็บก่อนกดยืนยัน ข้อมูลที่กรอกไว้จะหายไม่ถูกเก็บอัตโนมัติ

### ดูประวัติ / ดาวน์โหลด PDF / แก้ไขใบเก่า

ในแท็บ **📥 ประวัติ/PDF**:
- ใช้ตัวกรอง "กรองลูกค้า" / "กรองแบรนด์" หรือช่วงวันที่ Shipment เพื่อหาใบที่ต้องการ
- กด **📄 ดาวน์โหลด PDF** เพื่อพิมพ์หรือส่งให้ลูกค้า/ฝ่ายผลิต
- กด **✏️ แก้ไขข้อมูล** เพื่อเปิดใบเก่ามาแก้ไข แล้วกด "ยืนยันบันทึก" อีกครั้งเพื่อบันทึกทับของเดิม (เลขที่ใบงานเดิมจะไม่เปลี่ยน)
- กด **🗑 ลบ** เพื่อลบใบที่ไม่ต้องการ — ระบบจะถามยืนยันก่อนลบเสมอ
- ปุ่ม "📥 นำเข้า Excel" / "📊 ส่งออก Excel" ใช้แลกเปลี่ยนข้อมูลเป็นไฟล์ Excel ได้จากหน้านี้

### จัดการข้อมูลหลัก (Master Data)

แท็บ **⚙️ ข้อมูลหลัก** ใช้เพิ่ม/แก้ไข/ลบรายชื่อที่จะไปโผล่เป็นตัวเลือกในฟอร์ม (Mer, แบรนด์, ประเภทเสื้อผ้า, ชิ้นส่วน, ขั้นตอนการเย็บ, เครื่องจักร) โดยปกติไม่ต้องเข้ามาหน้านี้บ่อย เพราะพิมพ์ชื่อใหม่จากหน้ากรอกฟอร์มได้เลยตามข้อ 2 ด้านบน แต่ถ้าต้องการ **แก้ไขชื่อเดิมหรือลบรายการที่พิมพ์ผิด** ต้องมาทำที่หน้านี้ — ถ้ากดลบรายการที่มีใบงานใช้อยู่แล้ว ระบบจะเตือนก่อนว่ามีกี่ใบที่ใช้ชื่อนั้นอยู่
